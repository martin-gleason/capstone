...
Google's prepare phase is the process of collecting data sources and defining he metrics used to "answer the ask." To this endd, I looked to secure quality, open source data from the Chicago's Open Data portal. Because this data is well maintained by the city, and it does not involve propriety customer or client data, it should have been easy to use the [`RSocrata`](https://cran.r-project.org/web/packages/RSocrata/index.html) package. However, during the prepare phase, instead of exporting the Socrata query, I simply downloaded the entire data set.
...
This case study limts the data to two data sets: The [Divvy Ride Share](https://divvybikes.com) and the [City of Chicago Traffic Congestion by Region](https://data.cityofchicago.org/Transportation/Chicago-Traffic-Tracker-Congestion-Estimates-by-Re/t2qc-9pjd) data set.
...
congestion_2018 %>%
  ...
  gt::tab_header(title = gt::md("**Chicago Congetion Zone**")) |>
...
library(tidyverse)
library(gt)
options(scipen = 999) #elimiate scientific notation in R
